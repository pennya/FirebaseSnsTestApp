...
import com.duzi.duzisnstest.BuildConfig
...
import kotlinx.android.synthetic.main.activity_main_dummy.*
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        upload.setOnClickListener {
            ...
            val bitmap = BitmapFactory.decodeResource(resources, R.mipmap.ic_launcher) // 리소스로부터 비트맵 객체 불러옴
            val baos = ByteArrayOutputStream()
            bitmap.compress(Bitmap.CompressFormat.PNG, 100, baos) // 비트맵을 쓰기 스트림에 넣어줌
            val data = baos.toByteArray() // 스토리지에 파일을 만들고 버퍼 단위로 데이터를 저장하는 방식같음.
            firestorage?.reference?.child("imageFolder")?.child("ic_launcher2.png")
                ?.putBytes(data)?.addOnCompleteListener{ task ->
                    if(task.isSuccessful) {
                        println("업로드1 성공")
                    }
                }
            ...
            // 2. FileInputStream
            // 퍼미션 필요
            /*val path = Environment.getExternalStorageDirectory().absolutePath + "/teruten/sample_H1.mp4.MS4"
            val file = File(path)

            firestorage?.reference?.child("imageFolder")?.child("sample_H1.mp4.MS4")
                ?.putStream(FileInputStream(file))?.addOnCompleteListener{ task ->
                if(task.isSuccessful) {
                    println("업로드2 성공")
                }
            }*/
        ...
        download.setOnClickListener {
            firestorage?.reference?.child("imageFolder")?.child("ic_launcher2.png")
                ?.getBytes((1024 * 1024).toLong())?.addOnSuccessListener {  bytes ->
